...
<template>
  <div class="p-8 max-w-4xl mx-auto bg-gray-50 rounded-xl shadow-lg">
    <!-- Loading -->
    <div v-if="initialLoading" class="text-center text-gray-400 py-16">
      ⏳ {{ t('common.loading') }}
    </div>
    ...
    <!-- Error Fallback -->
    <div v-else class="text-center text-red-500 py-16">
      {{ t('common.error') || 'Не удалось загрузить профиль.' }}
    </div>
  </div>
</template>
...
const formattedDate = computed(() => {
  ...
  return d
      ? `${t('profile.registered')}: ${new Date(d).toLocaleDateString()}`
      : ''
})
