<template>
  <div class="max-w-3xl mx-auto p-6 space-y-6">
    ...
    <div class="flex flex-wrap gap-2 bg-gray-100 p-2 rounded">
      <button @click="format('bold')" class="px-2 py-1 border rounded hover:bg-gray-200">{{ t('posts.bold') }}</button>
      <button @click="format('italic')" class="px-2 py-1 border rounded hover:bg-gray-200">{{ t('posts.italic') }}</button>
      <button @click="format('underline')" class="px-2 py-1 border rounded hover:bg-gray-200">{{ t('posts.underline') }}</button>
      <button @click="format('strikeThrough')" class="px-2 py-1 border rounded hover:bg-gray-200">{{ t('posts.strikethrough') }}</button>
      <button @click="formatBlock('H1')" class="px-2 py-1 border rounded hover:bg-gray-200">{{ t('posts.h1') }}</button>
      <button @click="formatBlock('H2')" class="px-2 py-1 border rounded hover:bg-gray-200">{{ t('posts.h2') }}</button>
      <button @click="insertLink" class="px-2 py-1 border rounded hover:bg-gray-200">{{ t('posts.insertLink') }}</button>
      <button @click="triggerImageInput" class="px-2 py-1 border rounded hover:bg-gray-200">{{ t('posts.insertImage') }}</button>
    </div>
    ...
    <div class="flex justify-end gap-2">
      <button @click="cancel" class="px-4 py-2 border rounded">{{ t('common.cancel') }}</button>
...
onMounted(() => {
  if (!canPost.value) coordinator.navigateToGroup(group.value.id)
})
...
async function submit() {
  ...
  await store.createGroupPost(group.value.id, { title: postTitle.value, content: clean, topics })
  coordinator.navigateToGroupPosts(group.value.id)
}
</script>
